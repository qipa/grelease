...
####全局log文件
LOGFILE=/usr/local/atlassian-stash/external-hooks/log/post-receive-v2-`date +%Y%m%d`.log
####供用户查看的发布log
...
function_auto_pushlish_and_log (){
	...
	$ssh_publish_server "/backup/autoshell/yunniao_release_v2.sh $pass_branch_name" 
}
